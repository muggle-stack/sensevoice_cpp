    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            ortext_path = get_library_path()
        except Exception:
            import platform, pathlib, importlib.util
            lib_dir = pathlib.Path(
                importlib.util.find_spec("onnxruntime_extensions").origin
            ).parent / "lib"
            if platform.system() == "Darwin":
                ortext_path = lib_dir / "libortextensions.dylib"
            else:
                ortext_path = lib_dir / "libortextensions.so.0"
